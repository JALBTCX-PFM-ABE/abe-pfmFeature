...
void selectThread::select (OPTIONS *op, int32_t ph, PFM_OPEN_ARGS *oa, FEATURE **f, int32_t p, double ss)
{
...
{
  int32_t             percent = 0, old_percent = -1, numrecs, num_cols[3], **depth_count, **min_index, features_count;
  uint32_t            hpc_lines[1024];
  ...
  double              lat, lon, x_bin_size_degrees[3], x_meter_degrees[3], x_two_thirds_bin_degrees[3], min_dist[8];
  QString             string;
  ...
  int32_t pass = l_pass;
  double start_size = l_start_size;
  ...
  if (depth == NULL)
    {
      fprintf (stderr, "%s %s %s %d - depth - %s\n", options->progname, __FILE__, __FUNCTION__, __LINE__, strerror (errno));
      exit (-1);
    }

  depth_count = (int32_t **) calloc (3, sizeof (int32_t *));

  if (depth_count == NULL)
    {
      fprintf (stderr, "%s %s %s %d - depth_count - %s\n", options->progname, __FILE__, __FUNCTION__, __LINE__, strerror (errno));
      exit (-1);
    }

  min_index = (int32_t **) calloc (3, sizeof (int32_t *));

  if (min_index == NULL)
    {
      fprintf (stderr, "%s %s %s %d - min_index - %s\n", options->progname, __FILE__, __FUNCTION__, __LINE__, strerror (errno));
      exit (-1);
    }


  double bin_size = pow (2.0, (double) pass) * start_size;
  ...
  double y_meter_degrees = lat - options->mbr.min_y;

  double y_bin_size_degrees = y_meter_degrees * bin_size;

  double y_two_thirds_bin_degrees = y_bin_size_degrees * 2.0 / 3.0;

  double two_thirds_bin_meters = bin_size * 2.0 / 3.0;

  double bin_overlap_meters = two_thirds_bin_meters / 2.0;


  int32_t num_rows = (int32_t) ((options->mbr.max_y - options->mbr.min_y) / y_two_thirds_bin_degrees) + 1;
  ...
  int32_t save_size = (int32_t) ((options->mbr.max_x - options->mbr.min_x) / x_two_thirds_bin_degrees[0]) + 1;
  ...
  for (int32_t i = 0 ; i < 3 ; i++)
    {
      ...
      if (depth[i] == NULL)
        {
          fprintf (stderr, "%s %s %s %d - depth[i] - %s\n", options->progname, __FILE__, __FUNCTION__, __LINE__, strerror (errno));
          exit (-1);
        }


      depth_count[i] = (int32_t *) calloc (save_size, sizeof (int32_t));

      if (depth_count[i] == NULL)
        {
          fprintf (stderr, "%s %s %s %d - depth_count[i] - %s\n", options->progname, __FILE__, __FUNCTION__, __LINE__, strerror (errno));
          exit (-1);
        }


      min_index[i] = (int32_t *) calloc (save_size, sizeof (int32_t));

      if (min_index[i] == NULL)
        {
          fprintf (stderr, "%s %s %s %d - min_index[i] - %s\n", options->progname, __FILE__, __FUNCTION__, __LINE__, strerror (errno));
          exit (-1);
        }
      ...
      for (int32_t j = 0 ; j < save_size ; j++) depth[i][j] = NULL;
    }
  ...
  for (int32_t i = 0 ; i < 3 ; i++)
    {
      ...
      double startlat = options->mbr.min_y + (double) (i / 3) * y_two_thirds_bin_degrees;


      //  newlat is the latitude of the bottom of the current row.

      double newlat = options->mbr.min_y + (double) i * y_two_thirds_bin_degrees;
      ...
      num_cols[i] = (int32_t) ((options->mbr.max_x - options->mbr.min_x) / x_two_thirds_bin_degrees[i]) + 1;
      ...
      int32_t start_col = coord.x;
      int32_t start_row = coord.y;
      ...
      int32_t end_col = coord.x;
      int32_t end_row = coord.y;

      //qDebug () << __LINE__ << pass << i << start_col << end_col << start_row << end_row;

      for (int32_t j = start_row ; j <= end_row ; j++)
        {
          coord.y = j;

          for (int32_t k = start_col ; k <= end_col ; k++)
            {
              coord.x = k;
                              ...
                              if (options->zero || in_depth[m].xyz.z >= 0.0)
                                {
                                  ...
                                  double x_meter = (in_depth[m].xyz.x - options->mbr.min_x) / x_meter_degrees[i];
                                  double y_meter = (in_depth[m].xyz.y - startlat) / y_meter_degrees;


                                  //  These are the indices for the main bin that this point falls in (lower 2/3 of bin determines
                                  //  main bin).

                                  int32_t col = (int32_t) (x_meter / two_thirds_bin_meters);
                                  int32_t row = (int32_t) (y_meter / two_thirds_bin_meters);
                                  ...
                                  if (x_meter >= 0.0 && y_meter >= 0.0 && row >= 0 && row <= 3 && col >= 0 && col <= (num_cols[i] - 1))
                                    {
                                      ...
                                      if (col >= save_size)
                                        {
                                          fprintf(stderr,"%s %s %d %d %d\n",__FILE__,__FUNCTION__,__LINE__,col,save_size);
                                          exit (-1);
                                        }
#endif
                                      ...
                                      if (row < 3 || (row == 3 && fmod (y_meter, two_thirds_bin_meters) < bin_overlap_meters))
                                        {
                                          ...
                                          uint8_t x_overlap = NVFalse;
                                          uint8_t y_overlap = NVFalse;
                                              ...
                                              if (col)
                                                {
                                                  ...
                                                  if (depth[row][col - 1] == NULL)
                                                    {
                                                      fprintf (stderr, "%s %s %s %d - depth[row][col - 1] - %s\n", options->progname, __FILE__, __FUNCTION__,
                                              ...
                                              if (row)
                                                {
                                                  ...
                                                  if (depth[row - 1][col] == NULL)
                                                    {
                                                      fprintf (stderr, "%s %s %s %d - depth[row - 1][col] - %s\n", options->progname, __FILE__, __FUNCTION__,
                                                               __LINE__, strerror (errno));
                                                      exit (-1);
                                                    }
                                          ...
                                          if (x_overlap && y_overlap && row && col)
                                            {
                                              depth[row - 1][col - 1] = (POINT_DATA *) realloc (depth[row - 1][col - 1],
                                                                                                (depth_count[row - 1][col - 1] + 1) *
                                                                                                sizeof (POINT_DATA));

                                              if (depth[row - 1][col - 1] == NULL)
                                                {
                                                  fprintf (stderr, "%s %s %s %d - depth[row - 1][col - 1] - %s\n", options->progname, __FILE__, __FUNCTION__,
                                                           __LINE__, strerror (errno));
                                                  exit (-1);
                                                }
  ...
  for (int32_t i = 0 ; i < 3 ; i++)
    {
      for (int32_t j = 0 ; j < num_cols[i] ; j++)
        {
          //  Find the minimum depth.

          double min_depth = 999999999.0;
          min_index[i][j] = -1;

          for (int32_t k = 0 ; k < depth_count[i][j] ; k++)
            {

#ifdef __PFM_FEATURE_DEBUG__
              if (j >= save_size)
                {
                  fprintf(stderr,"%s %s %d %d %d\n",__FILE__,__FUNCTION__,__LINE__,j,save_size);
                  exit (-1);
                }
#endif
  ...
  for (int32_t i = 0 , p = 0; i < num_rows ; i++)
    {
      if (!i)
        {
          p = 0;
      ...
      for (int32_t j = 0 ; j < num_cols[p] ; j++)
        {
          int32_t start_row = qMax (p - 1, 0);
          int32_t end_row = qMin (p + 1, 2);

          int32_t start_col = qMax (j - 1, 0);
          int32_t end_col = qMin (j + 1, num_cols[p] - 1);
          ...
          if (min_index[p][j] >= 0 && depth_count[p][j])
            {
              ...
              double min_depth = depth[p][j][min_index[p][j]].xyz.z;
              float herr = depth[p][j][min_index[p][j]].h;
              float verr = depth[p][j][min_index[p][j]].v;


              memset (oct_hit, 0, sizeof (oct_hit));
              memset (oct_pos, 0, sizeof (oct_pos));
              memset (oct_herr, 0, sizeof (oct_herr));
              memset (oct_verr, 0, sizeof (oct_verr));
              for (int32_t oct = 0 ; oct < 8 ; oct++) min_dist[oct] = 9999999999999999.0L;

              double cut = order[options->order].x;
              if (min_depth > 40.0) cut = min_depth * order[options->order].y;

              for (int32_t k = start_row ; k <= end_row ; k++)
                {
                  for (int32_t m = start_col ; m <= end_col ; m++)
                    {
                      //  Now, determine octant in relation to the minimum for each depth point.

                      for (int32_t n = 0 ; n < depth_count[k][m] ; n++)
                        {

#ifdef __PFM_FEATURE_DEBUG__
                          if (m >= save_size)
                            {
                              fprintf(stderr,"%s %s %d %d %d\n",__FILE__,__FUNCTION__,__LINE__,m,save_size);
                              exit (-1);
                            }
#endif
                          ...
                          if (depth[k][m][n].xyz.z - min_depth >= cut)
                            {
                              ...
                              if (dist >= depth[k][m][n].h + herr)
                                {
                                  int32_t octant = get_octant (lon, lat, depth[k][m][n].xyz.x, depth[k][m][n].xyz.y);


                                  //  If we haven't exceeded our threshhold, increment the octant count.

                                  if (oct_hit[octant] < options->threshhold)
                                    {
                                      //  Find the closest octant point that exceeds our cutoff.  This will allow us
                                      //  to come up with an estimated shape for the feature.

                                      if (dist < min_dist[octant])
                                        {
                                          min_dist[octant] = dist;
                                          oct_pos[octant] = depth[k][m][n].xyz;
                                          oct_herr[octant] = depth[k][m][n].h;
                                          oct_verr[octant] = depth[k][m][n].v;
                                        }
              ...
              //  If we got our required number of hits in all 8 octants we are, obviously, good to go.  However, if we got our required number of
              //  hits in the following combinations of two octants we will also call it good:
              //
              ...
              for (int32_t oh = 0 ; oh < 8 ; oh++)
                {
                  if (oct_hit[oh] < options->threshhold)
                    {
                      good = NVFalse;

                      if ((oct_hit[0] >= options->threshhold || oct_hit[1] >= options->threshhold) &&
                          (oct_hit[2] >= options->threshhold || oct_hit[3] >= options->threshhold) &&
                          (oct_hit[4] >= options->threshhold || oct_hit[5] >= options->threshhold) &&
                          (oct_hit[6] >= options->threshhold || oct_hit[7] >= options->threshhold) &&
                          (oct_hit[7] >= options->threshhold || oct_hit[0] >= options->threshhold) &&
                          (oct_hit[1] >= options->threshhold || oct_hit[2] >= options->threshhold) &&
                          (oct_hit[3] >= options->threshhold || oct_hit[4] >= options->threshhold) &&
                          (oct_hit[5] >= options->threshhold || oct_hit[6] >= options->threshhold)) good = NVTrue;
              ...
              if (good)
                {
                  ...
                  if (features[pass] == NULL)
                    {
                      fprintf (stderr, "%s %s %s %d - features[pass] - %s\n", options->progname, __FILE__, __FUNCTION__, __LINE__, strerror (errno));
                      exit (-1);
                    }
                  ...
                  features[pass][features_count].bin_size = bin_size;
                  features[pass][features_count].confidence = 3;
                  features[pass][features_count].parent_record = 0;


                  //  Save the 6 to 8 closest points to the possible feature in each of 8 octants.  Also compute the max horizontal and vertical
                  //  errors for each octant hit.

                  float max_oct_herr = -1.0;
                  float max_oct_verr = -1.0;

                  for (int32_t oct = 0 ; oct < 8 ; oct++)
                    {
                      if (oct_hit[oct])
                        {
                          max_oct_herr = qMax (max_oct_herr, oct_herr[oct]);
                          max_oct_verr = qMax (max_oct_verr, oct_verr[oct]);
                        }
                  ...
                  //  We want to set the feature horizontal and vertical errors to the maximum horizontal and vertical errors for the shoal point
                  //  and all of the octant points.  The reason being that the feature is based on comparisons of all of the points so the errors
                  //  should be either the maximum or the average.  We're erring on the side of caution by using the maximum.

                  features[pass][features_count].h = qMax (herr, max_oct_herr);
                  features[pass][features_count].v = qMax (verr, max_oct_verr);


                  //  Compute the confidence based on the HPC (Hockey Puck of Confidence (TM)).

                  float v_cut = features[pass][features_count].v * 2.0;
                  float h_cut = features[pass][features_count].h * 2.0;

                  if (options->hpc)
                    {
                      int32_t hpc_count = 0;
                      int32_t hpc_line_count = 0;
                      for (int32_t k = start_row ; k <= end_row ; k++)
                        {
                          for (int32_t m = start_col ; m <= end_col ; m++)
                            {
                                  ...
                                  if (fabs (depth[k][m][n].xyz.z - features[pass][features_count].z) <= v_cut)
                                    {
                                      ...
                                      if (cut <= h_cut)
                                        {
                                          uint8_t hit = NVFalse;
                                          for (int32_t p = 0 ; p < hpc_line_count ; p++)
                                            {
      ...
      if (i)
        {
          for (int32_t j = 1 ; j < 3 ; j++)
            {
              for (int32_t k = 0 ; k < num_cols[j] ; k++)
                {

#ifdef __PFM_FEATURE_DEBUG__
                  if (k >= save_size)
                    {
                      fprintf(stderr,"%s %s %d %d %d\n",__FILE__,__FUNCTION__,__LINE__,k,save_size);
                      exit (-1);
                    }
#endif
                  ...
                  if (depth_count[j][k])
                    {
                      ...
                      if (depth[j - 1][k] == NULL)
                        {
                          fprintf (stderr, "%s %s %s %d - depth[j - 1][k] - %s\n", options->progname, __FILE__, __FUNCTION__, __LINE__, strerror (errno));
                          exit (-1);
                        }


                      for (int32_t m = 0 ; m < depth_count[j][k] ; m++) depth[j - 1][k][m] = depth[j][k][m];
          ...
          for (int32_t k = 0 ; k < num_cols[2] ; k++)
            {
          ...
          double newlat = options->mbr.min_y + (double) (i + (3 - p)) * y_two_thirds_bin_degrees;
          ...
          num_cols[2] = (int32_t) ((options->mbr.max_x - options->mbr.min_x) / x_two_thirds_bin_degrees[2]) + 1;
          ...
          int32_t end_col = coord.x;
          int32_t end_row = qMin (coord.y, open_args.head.bin_height - 1);
          ...
          double startlat = newlat - (2.0 * y_two_thirds_bin_degrees);
                                  ...
                                  if (options->zero || in_depth[m].xyz.z >= 0.0)
                                    {
                                      ...
                                      double x_meter = (in_depth[m].xyz.x - options->mbr.min_x) / x_meter_degrees[2];
                                      double y_meter = (in_depth[m].xyz.y - startlat) / y_meter_degrees;
          ...
          for (int32_t j = 0 ; j < num_cols[2] ; j++)
            {
              //  Find the minimum depth.
              ...
              for (int32_t k = 0 ; k < depth_count[2][j] ; k++)
                {
      ...
      percent = NINT (((float) i / (float) num_rows) * 100.0);
...
uint8_t selectThread::get_octant (double x0, double y0, double x1, double y1)
{
  float slope;
